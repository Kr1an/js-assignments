...
function getRectangleString(width, height) {
    ...
    let mdl = `│${' '.repeat(width-2)}│\n`.repeat(height-2);
    let btm = `└${'─'.repeat(width-2)}┘\n`;
    return top+mdl+btm;
}
...
function encodeToRot13(str) {
    return str.split('').map(function(a){
        ...
        if((cc>64 && cc<78) || (cc>96 && cc<110))
            return String.fromCharCode(cc + 13);
        else if((cc>109  && cc<123 ) || (cc>77  && cc<91))
            return String.fromCharCode(cc-13);
        else {
            return a;
        }
...
function isString(value) {
    return typeof value == 'undefined' || value == null ? false : typeof value.valueOf() == 'string';
}
...
function getCardId(value) {
    ...
    let firstValue = value.match(/\d+/)[0];
    let secondValue = value.match(/\D/)[0];

    firstValue = +firstValue-1;
    switch(secondValue){
        case "♣": return 13*0 + firstValue;
        case "♦": return 13*1 + firstValue;
        case "♥": return 13*2 + firstValue;
        case "♠": return 13*3 + firstValue;
    }
}
